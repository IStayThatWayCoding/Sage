...
module.exports = {
  structure: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("category")
        .setDescription("Information Category")
        .setRequired(true)
        .addChoices(
  ...
  run: async (bot, interaction) => {
    ...
    const resolveYT = await fetch(
      `https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${oasis}&key=${apiKey}`
    );
    ...
    function kFormatter(num) {
      ...
      return Math.abs(num) > 999
        ? Math.sign(num) * (Math.abs(num) / 1000).toString().match(regExp)[0] +
            "K"
    ...
    const resolveTWITCH = await fetch(
      `https://decapi.me/twitch/followcount/itsoasisgg`
    );
